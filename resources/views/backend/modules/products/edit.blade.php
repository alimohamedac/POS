...
@section('Title'){{ trans('backend/master.control-panel') }} - {{ trans('backend/products.edit') }}@endsection

@section('pageTitle'){{ trans('backend/products.edit') }}@endsection

@section('breadcrumb')
    <li><a href="{{ route('backend.products.index') }}">{{ trans('backend/products.control') }}</a></li>
    <li class="active">{{ trans('backend/products.edit') }}</li>
@endsection
            ...
            <div class="col-xs-12">
                <div class="box box-primary">
                    ...
                    <div class="row">
                        ...
                        <form action="{{ route('backend.products.update', $product->id) }}" enctype="multipart/form-data" method="post" role="form">
                            @csrf
                            <div class="col-md-6 col-xs-12">
                                <div class="box-body">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="text" name="name" value="{{ $product->name }}" class="form-control" id="name" placeholder="{{ trans('backend/products.hints.name') }}" required>
                                    </div>
                                    <div class="form-group">
                                        <label for="discreption">{{ trans('backend/products.attributes.discreption') }}</label>
                                        <textarea type="text" name="discreption" class="form-control" id="discreption" placeholder="{{ trans('backend/products.hints.discreption') }}">{{ $product->discreption }}</textarea>
                                    </div>
                                    <div class="form-group">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="number" name="purchase_price" class="form-control" value="{{ $product->purchase_price }}" id="purchase_price" required>
                                    </div>
                                    <div class="form-group">
                                        <label for="sale_price">{{ trans('backend/products.attributes.sale_price') }}</label>
                                        <input type="number" name="sale_price" step="0.01" class="form-control" id="sale_price" value="{{ $product->sale_price }}" required>
                                    </div>
                                    <div class="form-group">
